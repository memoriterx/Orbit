...
| 역할 | 한 마디로 | 무엇을 하나요? | 주요 스킬 |
|------|----------|--------------|----------|
| **leader (리드)** | 팀장 | 지시를 받고, 역할에 배분하고, 결과를 취합합니다. 코드는 직접 쓰지 않습니다. | `writing-plans`(설계자에 위임) · `skillify`(반복 감지) |
| **architect (설계자)** | 계획 작성자 | 구현 전 플랜 문서를 만듭니다. 당신이 승인한 뒤에야 구현이 시작됩니다. | `writing-plans` · `writing-skills` |
| **builder (구현자)** | 실제 개발자 | 승인된 계획대로 코드를 만듭니다. 테스트를 먼저 쓰고(TDD), 검증 후 완료를 선언합니다. | `test-driven-development` · `systematic-debugging` · `verification-before-completion` |
| **reviewer (검토자)** | 품질 보증 | 구현이 끝나면 완성도·동작·코드 품질을 3갈래로 검증합니다. | `GSD`(①) · `gstack`(②) · `requesting-code-review`(③) · `skillify` |
| **researcher (외부 조사자)** | 웹 탐색 전담 | 외부 문서·라이브러리·커뮤니티 패턴을 조사해 리드에게 보고합니다. 코드는 건드리지 않습니다. | —(역할 본연 기능) |
| **critic (비판자)** | 독립 검토자 | 위험한 계획이 있을 때만 호출됩니다. 설계자의 계획을 독립적으로 비판하고 PROCEED / REVISE를 판정합니다. | —(역할 본연 기능) |
| **explore (내부 탐색자)** | 코드베이스 탐색 전담 | 프로젝트 내 파일·코드 패턴을 찾아 보고합니다. researcher(외부)와 구분됩니다. 코드는 건드리지 않습니다. | —(역할 본연 기능) |
...
| ② 동작 | 실제 실행해서 기대대로 동작하는지 | gstack |
| ③ 품질 | 코드 정확성·보안·유지보수성 | requesting-code-review |
...
| `{{BEHAVIOR_VERIFICATION_METHOD}}` | Triple Crown ② 동작 검증 방법 (기본: gstack) |
| `{{MEMORY_PATH}}` | 프로젝트 메모리 파일 경로 |

researcher 역할은 도메인 무관 범용입니다. 어떤 기술 스택 프로젝트에도 동일한 researcher를 사용하고, 조사할 외부 소스 목록을 `{{RESEARCH_SOURCES}}` 슬롯에 채우기만 하면 됩니다. 슬롯이 비어 있으면 일반 웹 검색으로 대체됩니다.
