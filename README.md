...
리서처 역할은 **base에만 있다.** web-dev에는 researcher 에이전트가 없고, 검색 대상(소스 목록)만 공급한다.

| 역할 | 위치 | 설명 |
|------|------|------|
| 검색 엔진(역할) | `orbit-base/agents/researcher.md` | 도메인 무관 범용 리서처. 외부 소스를 조사해 리드에게 보고하는 읽기 전용 역할. 어떤 기술 스택에도 적용. |
| 검색 대상(소스 목록) | `orbit-web-dev/presets/research-sources.md` | base 리서처의 `{{RESEARCH_SOURCES}}` 슬롯을 채우는 소스 템플릿. 위치/지도 listing·리뷰 플랫폼·SNS 등 웹 비즈니스 외부 소스 예시. 실제 URL은 `<placeholder>`로 두고 프로젝트가 채움. |

```
orbit-base researcher (검색 엔진/역할, 도메인 무관)
        ↑ {{RESEARCH_SOURCES}} 슬롯에 주입
orbit-web-dev research-sources.md (검색 대상 = 위치/리뷰/SNS 예시 템플릿)
```

**비웹 프로젝트**(CLI·데이터 분석 등)는 orbit-web-dev 없이 orbit-base researcher만 쓰고, 해당 프로젝트에 맞는 소스를 직접 `{{RESEARCH_SOURCES}}`에 채우면 된다. 검색 역할이 base에 있는 이유다.
