...
| **architect (설계자)** | 계획 작성자 | 구현 전 플랜 문서를 만듭니다. 당신이 승인한 뒤에야 구현이 시작됩니다. | `opus` | `writing-plans` · `writing-skills` |
...
> 출처: https://wikidocs.net/book/19736
...
단, **미설치 시 검증이 약해집니다.** 완성도(①)와 품질(③)은 reviewer가 플랜·코드를 직접 읽어 수동으로 대조할 수 있지만 덜 체계적입니다. 동작 검증(②) 중 웹 UI 확인은 gstack(또는 사람)이 필요하며, 에이전트만으로는 어렵습니다(CLI·API는 직접 실행 가능). **제대로 된 자동화 검증을 원한다면 설치를 권장합니다. 없으면 검증이 수동으로 떨어지고, 특히 웹 UI 동작 검증은 제한됩니다.**
...
- [gstack](https://github.com/obra/gstack) — 브라우저·런타임 동작을 실제로 실행해 확인하는 QA 도구
- [gsd](https://github.com/obra/gsd) — 계획 대비 구현 완성도를 체계적으로 점검하는 도구
