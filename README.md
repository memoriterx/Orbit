...
- [이게 뭐예요?](#이게-뭐예요)
- [빠른 시작 — 3단계 설치](#빠른-시작--3단계-설치)
- [30분 만에 첫 사이클](#30분-만에-첫-사이클-처음-한-번-따라하기)
...
→ 설치 방법은 [동반 플러그인 설치](#동반-플러그인-설치) 참조.

| 동반 플러그인 | 검증 프롱 |
|--------------|----------|
| **superpowers** | ③ 품질 (`superpowers:requesting-code-review`) |
| **GSD** | ① 완성도 (`/gsd-verify-work`) |
| **gstack** | ② 동작 (`/qa`) |
...
처음 한 번만 보면 됩니다. 그 아래 "참조"는 지금 몰라도 됩니다.
...
위 "빠른 시작 → 요구사항" 또는 아래 [동반 플러그인 설치](#동반-플러그인-설치)를 참고해 먼저 설치하세요.
...
여러 작업을 한꺼번에 자동으로 돌리고 싶다면, 아래 "일하는 순서"의 **opt-in 자율 실행 모드**를 참고하세요(기본은 꺼져 있어, 매 작업마다 당신이 승인합니다).

막혔을 때는 아래 "[참조 → 막혔을 때](#막혔을-때-트러블슈팅)"를 확인하세요.
...
| 동반 플러그인(superpowers 등)이 없다고 표시됨 | 선택 플러그인 미설치 | **무시해도 됩니다.** orbit 핵심 방법론은 플러그인 없이 동작합니다. 자동화가 필요하면 아래 "동반 플러그인 설치" 참고 |
...
<details>
<summary>tmux 팀 환경 셋업 (선택 사항)</summary>
...
> 위 [빠른 시작 — 방법 2: 스크립트로 한 번에](#방법-2-스크립트로-한-번에)의 상세 설명입니다.
...
<details>
<summary>선택적 기능 부재 시 동작 (Graceful Degradation)</summary>
