...
- [ ] **[PERF-1] 병렬 Fan-out/Fan-in 패턴 정식화** (2026-06-20 발굴, 미착수)
  orbit은 이미 비공식적으로 병렬 백그라운드 dispatch를 함(예: explore+researcher 동시). 단 leader.md/SKILL에 1급 패턴으로 코드화 안 됨. 허브앤스포크 유지(리드 취합)·신규 인프라 0으로 explore+researcher 동시·Triple Crown ②③ 동시 검증 → 벽시간 단축. 자율모드·동시성 제어와 상호작용 → **architect 스파이크 필요**.
